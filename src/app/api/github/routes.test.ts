...
describe("GitHub route token 경계", () => {
  it.each([
    ...
    ["repository-meta", metadata, { owner: "octocat", repo: "hello-world", repositoryHasCommits: true }],
  ] as const)("%s는 쿠키가 없으면 auth_revoked를 반환한다", async (_name, handler, body) => {
    const response = await handler(request(body));
    expect(response.status).toBe(401);
    expect(await response.json()).toMatchObject({ kind: "auth_revoked" });
  });
});
...
describe("GitHub detail/meta route 응답", () => {
  ...
  it("commit-details 부분 실패 응답에서도 patch를 제거하고 진행 수치를 보존한다", async () => {
    ...
    expect(body).toMatchObject({ kind: "partial_failure", causeKind: "rate_limit", completed: 1, total: 2 });
    expect(body.partialCommits[0].files[0]).not.toHaveProperty("patch");
  });
  ...
  it("repository-meta 정상 응답에서 type, truncated, languages를 보존한다", async () => {
    ...
    const response = await metadata(authenticatedRequest({ owner: "octocat", repo: "hello-world", repositoryHasCommits: true }));
