...
describe("GitHub 인증 세션", () => {
  it("요청 쿠키의 암호화된 값에서만 토큰을 읽는다", () => {
    ...
    const request = new Request("https://example.com", { headers: { cookie: `${GITHUB_SESSION_COOKIE}=${encrypted}` } });
  ...
  it("변조된 쿠키는 auth_revoked 오류로 처리한다", () => {
    const request = new Request("https://example.com", { headers: { cookie: `${GITHUB_SESSION_COOKIE}=tampered` } });
    expect(() => getGitHubTokenFromRequest(request)).toThrow(expect.objectContaining({ kind: "auth_revoked" }));
  });
});
