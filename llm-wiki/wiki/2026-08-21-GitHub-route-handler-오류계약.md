...
브라우저는 GitHub API를 직접 호출하지 않고 `/api/github/*` 배치 route를 호출한다. PAT는 요청 body에 포함하지 않으며 서버가 암호화된 HttpOnly 쿠키에서 읽는다. 커밋 목록 커서는 첫 요청에서 고정한 head SHA, 인증 사용자 login, 다음 페이지를 base64url JSON으로 전달하고 서버에서 형태를 검증한다.
